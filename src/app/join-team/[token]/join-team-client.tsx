...
export function JoinTeamClient({ token }: { token: string }) {
    const router = useRouter();
    const [error, setError] = useState<string | null>(null);

    const joinMutation = trpc.teams.joinByToken.useMutation({
        onSuccess: (data) => {
            // Redirect to the newly joined team's page!
        ...
        return (
            <div className="flex min-h-screen items-center justify-center bg-gray-50">
                <div className="w-full max-w-md space-y-4 rounded-lg border bg-white p-8 shadow-sm text-center">
                    <h1 className="text-xl font-semibold text-destructive">Invalid or expired invite link</h1>
